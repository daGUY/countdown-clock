(function($) {
	$.fn.countdown = function(options) {
		...
		return this.each(function() {
			var countdown = $(this),
				initDays = Math.floor(settings.seconds / 86400),
		    ...
		    function displayTime() {
		        var days = Math.floor(settings.seconds / 86400),
		        	hours = Math.floor(settings.seconds / 3600) % 60,
		        	minutes = Math.floor(settings.seconds / 60) % 60,
					seconds = settings.seconds % 60;
		        
		        if (settings.padding) {
		        	if (initDays > 0) {
		        		countdown.html(pad(days) + ":" + pad(hours) + ":" + pad(minutes) + ":" + pad(seconds));	
		        	} else if (initHours > 0) {
